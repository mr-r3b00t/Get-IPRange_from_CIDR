# found online i think i made a quick mod as there was a scenario it didn't cater for...
# cant remember where i found it - but it's cool
...
$targets = Get-Content .\connect-targets.txt
...
elseif($subnet -like("*/31*")){
#write-host "Point to point Detected" -ForegroundColor Cyan
$subnet = $subnet -replace "/31",""
$subnet
$subnetOctet = $subnet -split "\."
$lastoct = $subnetOctet[3]
$lastoct = $lastoct -as [int]

$newlastoct = $lastoct + 1
$subnet = $subnetOctet[0] + "." + $subnetOctet[1] +"." + $subnetOctet[2] + "." + $newlastoct

$subnet
#read-host -Prompt "PAUSE"
}
else{
#write-host "use conversion maths" -ForegroundColor Green
Get-IpRange -Subnets $subnet
}

}


Get-IpRange -Subnets '192.168.5.0/24'
